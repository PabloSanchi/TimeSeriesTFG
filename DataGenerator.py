...
def find_closest_row(df, ws, wd, prev_values, ASC):
    # calculate distances
    df['distance'] = np.sqrt((df['wind_speed']-ws)**2 + (df['wind_direction_true']-wd)**2)
    ...
    for prev_ws, prev_wd in prev_values:
        df = df[(df['wind_speed'] != prev_ws) | (df['wind_direction_true'] != prev_wd)]
    ...
    return df #df['wind_speed'].values[0], df['wind_direction_true'].values[0]
